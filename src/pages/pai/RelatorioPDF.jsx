...
export default function RelatorioPDF() {
  ...
  const gerarPDF = async () => {
    ...
    try {
      ...
      const { montarRelatorioPDF } = await import('../../lib/relatorioPdf')
      const doc = new jsPDF('p', 'mm', 'a4')
      const mes = new Date().toLocaleString('pt-BR', { month: 'long', year: 'numeric' })
      const nome = child?.nome || 'Sem nome'

      // Todo o desenho vive em src/lib/relatorioPdf.js (função pura, testável
      // por render-pdf-relatorio.mjs). Aqui só juntamos os dados já calculados.
      montarRelatorioPDF(doc, {
